...
private:
	...
	void generate_coords(){
		...
		if (wall_status){
			for (int i=0; i<number_of_particles; i++){
				velocities[i][0] += wall_coord/(tau*10000)*2.5; //max_step здесь принято за 1E5
			}
		}
	...
	std::vector<std::vector<double>> m_accelerations(){
		...
		generate_field();
		long int local_n_o_p = number_of_particles/number_of_cores;
...
public:
	...
	void slow_evolve(){
		...
		std::ofstream load("results/loading.txt");
	...
	std::vector<double> velocity_of_CM(long int stp){
		std::vector<double> velocity_of_CM(3);
		std::vector<std::vector<double>> vels = velocities(stp);
		for (long int i=0; i<number_of_particles; i++){
			velocity_of_CM += vels[i];
		}
		return velocity_of_CM;
	}
	...
	double energy(long int stp){
		//print("slow call")
		double U = 0;
		for (long int i =0; i<number_of_particles; i++){
			for (long int j =0; j<number_of_particles; j++){
				if (i>j){
					double R6 = std::pow(sum(power(data[stp][i]-data[stp][j], 2)),3);
					U += force_size*(1/(2*R6)-1)/R6;
				}
			}
		}

		U = (10*U)/6;
		double T = 1/2*sum(power(velocities(stp), 2));
		return U+T;
	}
